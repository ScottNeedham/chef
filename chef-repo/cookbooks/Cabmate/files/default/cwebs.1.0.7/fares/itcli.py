...
class Fare():
    ...
    def updateFare(self, dic ):
        try:
            if dic.has_key("order_id") and dic["order_id"] != "":
             self.IBSorderID = format_field.format_field(str(dic["order_id"]), 11, True)
        ...
        try:
            if dic.has_key("operator_id") and dic["operator_id"] != "":
                if dic["operator_id"] > 99 :
                    self.OperatorID = format_field.format_field(str(dic["operator_id"]), 4, True)
    ...
    def __init__(self, fare_dic, zone, SeqNo, fn=''):
        ...
        if "account_number" in fare_dic:
            self.acctno = format_field.format_field(fare_dic["account_number"], 12, True)
        ...
        if fare_dic.has_key("customer_number"):
            if fare_dic["customer_number"] != "":
                try:
                    ...
                    self.custno = format_field.format_field(fare_dic["customer_number"], 10, True)
        ...
        if lCorporateAccount:
            if fare_dic.has_key('account_name'):
                self.acctname = 20*' ' if fare_dic['account_name'] == '' else format_field.format_field(fare_dic['account_name'], 20, True)
            else:
                self.acctname = 20*' ' 
        else:
            if "account_number" in fare_dic:
                self.acctname = format_field.format_field("TEL" + fare_dic["account_number"], 20, True) 
            #print 'self.acctname ', self.acctname
        # Telephone Number
        self.phone = 19*' '  
        try:
            if "phone" in fare_dic:
                self.phone = format_field.format_field(fare_dic["phone"], 19, True)
        except Exception as e:
            self.phone = 19*' '               
        # Passenger Number
        try:
            self.passengername = format_field.format_field(fare_dic["passenger_name"], 32, True)
        except Exception as e:
            self.passengername = 32*' '      
        # pickup Street Number
        try:
            self.pick_street_no = format_field.format_field(fare_dic["pick_up_street_number"], 6, True)
        except Exception as e:
            self.pick_street_no = 6*' '
        try:
            self.pick_street_name = format_field.format_field(fare_dic["pick_up_street_name"], 32, True)
        except Exception as e:
            self.pick_street_name = 32*' '  
        self.pick_street_type = 2*' '   
        
        try: 
            self.pick_city = format_field.format_field(fare_dic["pick_up_city"], 4, True)
        except Exception as e:
            self.pick_city = 4 *' '
        try:
            self.pick_apartment = format_field.format_field(fare_dic["pick_up_apartment_number"], 7, True)
        except Exception as e:
            self.pick_apartment = 7*' '
        # this is access code 
        try:
            self.pick_code = format_field.format_field(fare_dic["pick_up_apartment_code"], 7, True)
        except Exception as e:
            self.pick_code = 7*' '
        ...
        try:
            self.pbuildingname = format_field.format_field(fare_dic["pick_up_building_name"], 32, True)
        except Exception as e:
            self.pbuildingname = 32*' '

        # Destination Street Number
        try:
            self.dest_street_no = format_field.format_field(fare_dic["destination_street_number"], 6, True)
        except Exception as e:
            self.dest_street_no = format_field.format_field(' ', 6, True)
        try:
            self.dest_street_name = format_field.format_field(fare_dic["destination_street_name"], 32, True)
        except Exception as e:
            self.dest_street_name = format_field.format_field(' ', 32, True)    
        self.dest_street_type = 2*' '
        try:
            self.dest_city = format_field.format_field(fare_dic["destination_city"], 4, True)
        except Exception as e:
            self.dest_city = 4*' '    
        ...
        try:
            self.dbuildingname = format_field.format_field(fare_dic["destination_building_name"], 32, True)
        except Exception as e:
            self.dbuildingname = 32*' '
    ...
    # Number of passengers
        ...
        try:
            ...
            if len(fare_dic["remarks"]) > 0:
                ...
                for rem, i in zip(fare_dic["remarks"], range(len(fare_dic["remarks"])) ):
                    if i == 0:
                        self.remarks1 = format_field.format_field(rem, 32, True)
                    elif i == 1:
                        self.remarks2 = format_field.format_field(rem, 32, True)
                    elif i == 2:
                        self.remarks3 = format_field.format_field(rem, 32, True)
                    elif i == 3:
                        self.remarks4 = format_field.format_field(rem, 32, True)
                    else:    
                        break
        ...
        try:
            if 'fleet_number' in fare_dic:
                ...
                self.fleet_number = format_field.format_field(str(fare_dic["fleet_number"]), 4, False);
        except Exception as e:
            self.fleet_number =  4*' '
        ...
        try:                   
            ...
            self.fare_amount = format_field.format_field(("%2.2f" % (self.fare_amount_i / 100.0)), 8, True);
        except Exception as e:
            self.fare_amount = 8*' '
        ...
        self.room_number = 8*' '
        try:
            self.contact_name = format_field.format_field(fare_dic["contact_name"], 20, True);
        except Exception as e:
        ...
        try:
            self.pcityname = format_field.format_field(fare_dic["pick_up_city"], 32, True)
        except KeyError:
            self.pcityname = 32*' '

        try:
            self.dcityname = format_field.format_field(fare_dic["destination_city"], 32, True)
        except KeyError:
            self.dcityname = 32*' '
        ...
        if fare_dic.has_key("dispatch_priority"):
            try:
                ...
                if dispatch_priority in range(10):
                    self.dispatch_priority = format_field.format_field(str(dispatch_priority), 4, True)   
            except Exception as e:
                pass     
        print 'self.dispatch_priority= %s ' % ( self.dispatch_priority)
        try:
            self.dest_apartment = format_field.format_field(fare_dic["destination_apartment_number"], 7, True)
        except Exception as e:
        ...
        try:
            self.dest_code = format_field.format_field(fare_dic["destination_apartment_code"], 7, True) 
        except Exception as e:
            self.dest_code = 7*' '  
        ...
        if "prompts" in fare_dic :
            for p, i in zip(fare_dic["prompts"], range(len(fare_dic["prompts"]))):
                ...
                self.ei_Conf_Prompt[i] = format_field.format_field(p["response"], 13, True)
                special_data = special_data + format_field.format_field(p["response"], p["length"], True) 
                self.special_data = format_field.format_field(special_data, 80, True)  
        try:
        ...
        try:
            ...
            if fare_dic.has_key("order_id") and fare_dic["order_id"] != "":
                self.IBSorderID = format_field.format_field(str(fare_dic["order_id"]), 11, True)
        except Exception as e:
            self.IBSorderID = 11*' '
        try:
            self.mobile_phone = format_field.format_field(fare_dic["mobile"], 18, True)
        except Exception as e:
                    ...
                    if fare_dic.has_key("bid_info") and self.Bid == 'Y':
                        if len(fare_dic["bid_info"]) > 0:
                            for rem , i in zip(fare_dic["bid_info"], range(len(fare_dic["bid_info"])) ):
                                self.bid_info[i] = format_field.format_field(rem, BIDINFO_ENTRY_LEN, True)
            except KeyError:    
                pass
            self.filler= 329*' '
    ...
    def create_space_saver_struct(self):        
        ...
        long matching_fare;
        long orig_time;
        short ie_fleet[REDISP_FLEET];               // 10
        long start_date;     
        short recycled[68];  
        short mod_uid;
        char hol_pat;
        unsigned char reg_stop[NWEEKDAYS];
        char ie_type;
        char modem;
        char spare;
        '''
    ...
    def fare_to_bin(self, pzone=None, seqno=None):
        ...
        if self.SMS_Enabled == 'Y' and custno > 0:
            self.flag |=  FareConstant.SMS_ENABLED  
            # if no mobile phone
            if self.mobile_phone != 18*' ':
                xphones[3] = self.mobile_phone
            elif  self.phone == 19*' ':
                xphones[3] = self.acctno
            else:
                xphones[3] = self.phone
        ...
        ##ss data
        #'24I 7I 10H I 69H 194c'
        #print ' ss_data {0} '.format( self.ss_data )
        ...
        self.acctno[:11] + b'\x00' ,        # acct_num
        self.acctname[:19] + b'\x00',       # acct_name
 
        0,                                  # codemsgctr
        self.num_of_jobs,
        0,                                  # modem           

        self.driver_name[:23] + b'\x00', # 24s
        self.Flag2,
        ...
        0,                          # self.weight,
        self.Flag1,
        self.landmark + b'\x00',    # self.landmark, #33s
        self.borough + b'\x00',     # self.borough,
        0, #self.cc_rec_num,
        self.ei_conf_length[0], self.ei_conf_length[0], self.ei_conf_length[2], self.ei_conf_length[3], #self.ei_conf_length],  # 111 * ' '+ b'\x00', # ei_conf
        self.ei_conf_length[4], self.ei_conf_length[5], self.ei_conf_length[6], self.ei_conf_length[7],
        ''.join([i for i in self.ei_conf_prompt]),  #104*' ',
        120 * b'\x00',
        160 * b'\x00', # 10 * 16 cvd_resp
   

        4 * b'\x00',          # cvd_call_cycle
        2 * b'\x00',          # q_posn
        60 * b'\x00',         # airport
        0,                    # CoPay
        ...
        ' 3b B 20s 3s 8s 9s 20s I I I H H I I I 100s ' ,  # 192s = payment
        ' I' , #    # veh_class_32
        ' 16s' , 
        ' 1B 1B H H H I I ' ,   # rgev    

        self.ss_fmt,             # '24I 7I 10H I 69H B 7c 3B 183B' #'H B B 2I 468s' , # ss
        '198s 1338s ',           #' 1536s ' = xjobinfo
        '396s 16s' ,      
...
def create_fare_to_bin(pzone=None):
    ...
    fmts =  [ 
        '16I',
        '2i h H 4s 8s 8s 34s 34s 34s 1H 8s 8s 33s 3c',  # addr_struct addr_extra
        '2i h H 4s 8s 8s 34s 34s 34s 1H 8s 8s 33s 3c',
        '66s',       #forminfo
        '66s',
        '20s' ,         #phone
        '80s 16',       # user_data
        '4I' ,          # fdv_types
        '2H',           #  combat_vals 704
        '2B 10H 2B',       #  redisp_fltinfo 708 *****PADDING 2B ****
        '4B 20s 3B 8s 9s 20s 3I 2H 3I 100s',            # ttab_struct
        'I',            # veh_class_32
        '1H 2B 1I 2f' ,     #  g_event_xy
        '1H 2B 1I 2f',      #code_77_event
        '480c',             # //12 * 40(NUM_FARE_EVENTS=40) = 480c
         # xpickups = 1536s
        '1536s',
        #'2I 2H 4s 8s 8s 34s 34s 34s 1H 8s 8s 33s 3c 1B 65s 2B',
        #'2I 2H 4s 8s 8s 34s 34s 34s 1H 8s 8s 33s 3c 1B 65s 2B',
        #'2I 2H 4s 8s 8s 34s 34s 34s 1H 8s 8s 33s 3c 1B 65s 2B',
        #'2I 2H 4s 8s 8s 34s 34s 34s 1H 8s 8s 33s 3c 1B 65s 2B',
        #'2I 2H 4s 8s 8s 34s 34s 34s 1H 8s 8s 33s 3c 1B 65s 2B',
        ...
        '396s',             # extraforminfo
        '4I',               # reg_track
        '16H I 2B 132s 132s 12s 20s 3B 24s 5B 33s 3B 1I',
        '8s 104s ', # ei_config
        ' 120s' , # xphones
        '3H 2B 2I 3H 2B 2I 3H 2B 2I 3H 2B 2I 3H 2B 2I 3H 2B 2I 3H 2B 2I 3H 2B 2I 3H 2B 2I 3H 2B 2I' , # cvd_resp * 10
        '4B',  # pad_share
        '2B',
        '17s 7s 11s 9s 16s ' , # airport_info
        'H', # CoPay
    ...
    for item in fmts:       
        ...
        print  ' fmt size = %d total = %d'  % ( struct.Struct(item).size, total  ) 
       
    rec_fmt = ''.join(fmts)
    rec_size = struct.Struct(rec_fmt).size
    print ' total rec size = %d ' % ( rec_size )



    SeqNo = socketcli. sClient.generate_seqno()
    zone_info = { "zone" : 102}
    dic = {     u'pick_up_street_name': u'11th Avenue',
                u'addresses': [],
                u'destination_street_number': u'1689',
                u'payment_method': u'account',
                u'pick_up': u'11th Avenue New York ',
                u'destination_building_name': u'',
                u'fleet_number': 1,
                u'driver_id': u'8080',
                u'pick_up_apartment_number': u'',
                u'pick_up_apartment_code': u'',
                u'pick_up_lat': 40.7176533,                
                u'vehicle_number': u'',
                u'destination': u'1689 1st Avenue New York 10128 ',
                u'destination_city': u'New York',
                u'flat_rate': 4800,
                u'pick_up_city': u'New York',
                u'txt_back': False,
                u'pick_up_lng': -74.008655999999974,
                u'pick_up_lat': 40.7176533,               
                u'quick_return': u'Y',
                u'destination_street_name': u'1st Avenue',
                u'pick_up_passengers': 1,
                u'dest_apartment_code': u'',
                u'phone': u'5435435435',
                u'pick_up_building_name': u'',
                u'remarks': [u'9999-B', u'', u'', u''],
                u'dispatch_priority': 9,
                u'destination_zip_code': u'10128',
                u'job_authorization': 0,
                u'mobile': u'',
                u'callout_not_allowed': u'',
                u'pick_up_street_number': u'',
                u'customer_number': u'',
                u'destination_vehicles': 1,
                u'prompts': [],
                u'booker': u'',
                u'account_number': u'5435435435',
                u'will_call': u'Y',
                u'pick_up_zip_code': u'10128',
                u'passenger_name': u'',
                u'assign': False,
    }

    #sys.stdout.write("%s:  new fare seqno  %s\n" % (datetime.datetime.strftime(datetime.datetime.now(), Config.LOG_DATETIME_FORMAT), SeqNo )) 
    new_fare = Fare(dic, zone_info["zone"], SeqNo)

    d , s = new_fare.fare_to_bin()
    return d,s
...
if __name__ == "__main__":
    ...
    import struct, sys
    #create_fare_to_bin()
    m_format = '8I 2f 7H 2c 33s 33s 33s 33s 2c H H 64s 10s 2H' 
    s = struct.Struct(m_format)
    print  ' s.size ==>' , s.size


    #seqno = '2698460545'
    data= b''

    m_format = 'I I I I I c c c c H H 64s 2H 10s 4H 134s'
    #m_format = 'I I I I I c c c c 196s'
    s = struct.Struct(m_format)   
    ...
    try:
        fare_dic = {}
        fare_dic["pickup_datetime"] = '2016-3-5 17:46' # '20160616 14:30'
        pickup_datetime = datetime.datetime.strptime(fare_dic["pickup_datetime"], Config.DateFormat_SaveOrder_Input)
        resv_date = pickup_datetime.strftime('%Y%m%d')
        resv_time = pickup_datetime.strftime('%H:%M')

        
        reginfo_disp_time = int( time.mktime(pickup_datetime.timetuple()))
        print resv_date
        print resv_time
        print pickup_datetime
        print reginfo_disp_time

        resv = pickup_datetime.timetuple()
      

        print ' pickup tuple ', resv 

        print ' int pickup ', reginfo_disp_time

        fare_dic["pick_up_zip_code"] = "89000"
...
#SP_multi_fleet_zoning_on
#SP_fleet_related_op_on
# Retrieve AP_default_fleet
#SP_CabWeb_with_direction
#