...
def _dispatch_modify_book_order_tfc(sockcli, data_src_repo, zonecache):
    try:  
        ...
        #No fare in DB, return an error
        if lim_fare == False and lfu_fare == False:
            sys.stdout.write("%s: returning 500 because we could not find fare\n" % (datetime.datetime.strftime(datetime.datetime.now(), Config.LOG_DATETIME_FORMAT)) )                
            response.status = 500
            res["result"] =  {"job_number": order_dic["fare_number"]}
            return Utils.make_error_response(response, 500, ErrorMsg.ERROR_MSG_INVALID_FARE_CABMATEREPO, res)               
        ...
        new_fare = FareFis(order_dic, zone_info["zone"], SeqNo, fare_num) #itcli.Fare(order_dic, zone_info["zone"], SeqNo, fare_num)   
        errmsg=None      
        try:
...
def send_to_host(new_fare, order_dic, zone, SeqNo, errmsg, lWillCall, fare_num):
    try:
        ...
        try:
            ...
            if SeqNo is None:
                SeqNo = socketcli. sClient.generate_seqno()
                new_fare = FareFis(order_dic, zone, SeqNo, fare_num) # itcli.Fare(order_dic, zone, SeqNo, fare_num)            
