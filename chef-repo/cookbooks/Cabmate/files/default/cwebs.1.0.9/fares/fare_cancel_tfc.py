...
def send_cancel_request(sockcli, job_number, fare_type, order_dic, response):
    ...
    try:
        ...
        try:   
            ...
            dic["zone"] = -1
            if fare_type in [ "immediate", "future"] :
                stored_fare = fare_container_db( fare_type=fare_type)
            ...
            if  dic["zone"] < 0:
                ...
                return response.status,  ErrorMsg.ERROR_MSG_INVALID_FARE_CABMATEREPO, res
            ...
            response.status, resp_message, res = send_cancel_to_host(its_evt, SeqNo, dest, mt,  nowait=True)

            if dest == msgconf.DM :
                dest = msgconf.Q_PROC
                mt =  msgconf.MT_CNCL_QENTRY
                rs,  rm, r = send_cancel_to_host(its_evt, SeqNo, dest, mt,  nowait=True)
