...
def updateTripEntry(dic):
    
    sys.stdout.write("%s: entering %s\n" % (datetime.datetime.strftime(datetime.datetime.now(), Config.LOG_DATETIME_FORMAT), __name__))
   
    try:
        errmsg = "OK"
        must_k = ["full_dropoff_address", "full_pickup_address", "job_number"]
        for i in must_k:
            if i not in dic:
                return "No data to store"
        try:
            db = smalldb.DB(Config.DB_HOST, Config.DB_USER, Config.DB_PASSWORD, 'cabmate')
            db.connect()    
            sql_stat = '''
                UPDATE trip
                SET FullPickupAddress= \'%s\',
                FullDropoffAddress= \'%s\'
                WHERE FareNumber = \'%u\'
                ;
                ''' % (
                    dic["full_pickup_address"],
                    dic["full_dropoff_address"],
                    dic["job_number"]
                )
            ...
            sys.stdout.write("%s: updateTripEntry exception %s \n" % (datetime.datetime.strftime(datetime.datetime.now(), Config.LOG_DATETIME_FORMAT), e ))
            errmsg =  str(e)
...
def get_cabwin_version(data_src): 
    cab_win_version = (17,0,0)
    try:
        sql_stat = '''SELECT major, minor, revision from version where process = 'Cabwin';'''
        info = data_src.fetch_many(sql_stat)
        if info != None and len (info) > 0:
            return info[0]  
    except Exception as e:
        sys.stdout.write("%s: get_cabwin_version exception %s \n" % (datetime.datetime.strftime(datetime.datetime.now(), Config.LOG_DATETIME_FORMAT), e ))

    return cab_win_version



def get_fare_info(data_src, fare_num, fare_type):
    
    taxi=-1
    zone=-1
    fare_exists = False
    table_to_read = "farefl"
    if fare_type == "future":
        table_to_read = "futfarefl"
 
    if type(fare_num) in [unicode, str] and len(fare_num) > 7:
        fare_stat = "where FareNumber = %s limit 1 " % (fare_num)
    else:
        fare_stat = " where MOD(FareNumber, 1000000) = %s limit 1 " % (fare_num)
    
    sql_stat1 = '''
            select Taxi as taxi,                          
            Pickup_DispatchZone as zone, 
            FareNumber as fare_number from  %s 
            ''' % (table_to_read)
    
    sql_stat = sql_stat1 + fare_stat

    print sql_stat
    info = data_src.fetch_many(sql_stat)

    if len(info) > 0:        
        taxi, zone, fare_num = info[0]
        sys.stdout.write("%s: get_fare_info %s taxi=%d zone=%d\n" % (datetime.datetime.strftime(datetime.datetime.now(), Config.LOG_DATETIME_FORMAT), fare_num, taxi, zone))
        
    return (fare_num, taxi, zone)

def read_fare_record(fare_num, fare_type):
    try:
        f, t, z = (-1, -1, -1)
        data_src= smalldb.DB(Config.DB_HOST_REPO, Config.DB_USER_REPO, Config.DB_PASSWORD_REPO, Config.DB_NAME_REPO)
        data_src.connect() 
        #Returns fare_num, zone, taxi      
        f, t, z = get_fare_info ( data_src, fare_num, fare_type)

        data_src.close()
    except Exception as e:
        sys.stdout.write("%s: read_fare_record %s taxi=%d zone=%d\n" % (datetime.datetime.strftime(datetime.datetime.now(), Config.LOG_DATETIME_FORMAT), fare_num, t, z))
        data_src.close()
...
if __name__ == "__main__":
    ...
    try:
        dic = { "FullPickupAddress" : "HINES STRAAS , 456 NORTH YORK, ONTARIO  ", 
                "FullDropoffAddress" : "675 6TH STREET , OTTAWA ,ONTARIO, CANADA" , 
                "job_number" : 5662}
        res = updateTripEntry(dic)
    ...
    data_src= smalldb.DB('127.0.0.1', 'taxi', 'taxi', "CabmateRepo")
    data_src.connect()
    fare_num="201610110006626"
    fare, taxi, zone = get_fare_info(data_src, fare_num, "immediate")
    print ' Got %s %d %d ' % (  fare, taxi, zone )
