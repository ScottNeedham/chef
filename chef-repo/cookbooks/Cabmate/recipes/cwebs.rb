...
Chef::Log.info(cwebsversion2(node["ipaddress"],octet,"1.0.7"))
remote_directory '/cwebs/cwebs' do
	...
	not_if do cwebsversion2(node["ipaddress"],octet,"1.0.7") end 

#	not_if { File.readlines('/cwebs/cwebs/build/build.py').grep(/1.0.7/).size > 0}
end
